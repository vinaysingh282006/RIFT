...
import { DrugRisk, mockJsonOutput } from "@/lib/mockData";
...
export function JsonOutputPanel({ results }: JsonOutputPanelProps) {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);

  const displayData = results ? {
    analysis_id: "PG-" + Math.random().toString(36).substr(2, 9).toUpperCase(),
    timestamp: new Date().toISOString(),
    vcf_stats: {
      variants_count: results.vcf.variantCount,
      metadata_lines: results.vcf.metadata.length
    },
    pharmacogenomic_results: results.risks.map(r => ({
      drug: r.drug,
      phenotype: r.phenotype,
      risk: r.risk,
      recommendation: r.clinicalNote
    }))
  } : mockJsonOutput;
